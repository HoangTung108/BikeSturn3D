...
{
	[Obsolete("Use SplineController instead!")]
	public class FollowSpline : CurvyComponent
	{
		...
		protected void UpdateRelative()
		{
			...
			{
				base.Transform.position = Spline.MoveFast(ref mCurrentTF, ref Current.m_Direction, Speed * DTTime.deltaTime, Clamping);
			}
			else
			{
				base.Transform.position = Spline.Move(ref mCurrentTF, ref Current.m_Direction, Speed * DTTime.deltaTime, Clamping);
			}
			if (SetOrientation)
			{
				orientate();
		...
		protected void UpdateAbsoluteExtrapolate()
		{
			if (FastInterpolation)
			{
				base.Transform.position = Spline.MoveByFast(ref mCurrentTF, ref Current.m_Direction, Speed * DTTime.deltaTime, Clamping);
			}
			else
			{
				base.Transform.position = Spline.MoveBy(ref mCurrentTF, ref Current.m_Direction, Speed * DTTime.deltaTime, Clamping);
			}
			if (SetOrientation)
			{
				orientate();
		...
		{
			base.Transform.position = Spline.MoveByLengthFast(ref mCurrentTF, ref Current.m_Direction, Speed * DTTime.deltaTime, Clamping);
			if (SetOrientation)
			{
				orientate();
			}
