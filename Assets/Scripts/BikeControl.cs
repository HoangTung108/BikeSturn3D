...
public class BikeControl : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		...
		{
			bikeSounds.IdleEngine.volume = Mathf.Lerp(bikeSounds.IdleEngine.volume, 0.5f, 0.1f);
			bikeSounds.LowEngine.volume = Mathf.Lerp(bikeSounds.LowEngine.volume, 0.1f, 0.1f);
			bikeSounds.HighEngine.volume = Mathf.Lerp(bikeSounds.HighEngine.volume, 0f, 0.1f);
			return;
		}
		bikeSounds.IdleEngine.volume = Mathf.Lerp(bikeSounds.IdleEngine.volume, 1.6f - Pitch, 0.1f);
		if ((Pitch > PitchDelay || accel > 0f) && shiftTime == 0f)
		{
			bikeSounds.LowEngine.volume = Mathf.Lerp(bikeSounds.LowEngine.volume, 0f, 0.2f);
			bikeSounds.HighEngine.volume = Mathf.Lerp(bikeSounds.HighEngine.volume, 0.5f, 0.2f);
		}
		else
		{
			bikeSounds.LowEngine.volume = Mathf.Lerp(bikeSounds.LowEngine.volume, 0.5f, 0.2f);
	...
	private void OnCollisionEnter(Collider other)
	{
		...
		{
			if (other.gameObject.tag == "floorcrash")
			{
				crash = true;
			}
		}
		...
		else if (Application.loadedLevel > 16 && other.gameObject.tag == "floorcrash")
		{
			crash = true;
		}
	}
